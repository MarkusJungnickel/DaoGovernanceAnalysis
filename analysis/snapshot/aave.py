...
###################### READ CSVS ###########################
proposals = genfromtxt('../../Snapshot/aave/proposals.csv',
                       delimiter="//")
...
proposalsStr = genfromtxt(
    '../../Snapshot/aave/proposals.csv', delimiter="//", dtype=str)
...
s2 = sns.jointplot(x=propOutcomes[:, 4], y=propOutcomes[:, 1], kind="hex",
                   norm=mpl.colors.LogNorm(), marginal_kws=dict(bins=10), color="#4CB391")
...
voters = []
votesByProposal = []
with open('../../Snapshot/aave/votes.csv', "r") as f:
...
ax4.hist(np.sort(values), edgecolor='white', linewidth=1.2, color="#4CB391")
ax4.set_ylabel("# of Members")
ax4.set_xlabel("Degree Centrality")

# centrality analysis
fig5, ax5 = plt.subplots()
ax5.hist(votingFrequency, edgecolor='white', linewidth=1.2, color="#4CB391")
ax5.set_yscale("log")
ax5.set_ylabel("Freq")
